/*
 * ShipBob Developer API
 *
 * ShipBob Developer API Documentation
 *
 * API version: 1.0
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package shipbob

import (
	"encoding/json"
)

// ReturnsCreateReturnViewModel struct for ReturnsCreateReturnViewModel
type ReturnsCreateReturnViewModel struct {
	FulfillmentCenter ReturnsFulfillmentCenterViewModel `json:"fulfillment_center"`
	// Array of inventory items being returned
	Inventory []ReturnsReturnInventoryViewModel `json:"inventory"`
	// Shipment from which the items in the return originated
	OriginalShipmentId NullableInt32 `json:"original_shipment_id,omitempty"`
	// Client-defined external unique identifier for the return order
	ReferenceId NullableString `json:"reference_id"`
	// Tracking number for the return shipment
	TrackingNumber NullableString `json:"tracking_number,omitempty"`
}

// NewReturnsCreateReturnViewModel instantiates a new ReturnsCreateReturnViewModel object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewReturnsCreateReturnViewModel(fulfillmentCenter ReturnsFulfillmentCenterViewModel, inventory []ReturnsReturnInventoryViewModel, referenceId NullableString, ) *ReturnsCreateReturnViewModel {
	this := ReturnsCreateReturnViewModel{}
	this.FulfillmentCenter = fulfillmentCenter
	this.Inventory = inventory
	this.ReferenceId = referenceId
	return &this
}

// NewReturnsCreateReturnViewModelWithDefaults instantiates a new ReturnsCreateReturnViewModel object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewReturnsCreateReturnViewModelWithDefaults() *ReturnsCreateReturnViewModel {
	this := ReturnsCreateReturnViewModel{}
	return &this
}

// GetFulfillmentCenter returns the FulfillmentCenter field value
func (o *ReturnsCreateReturnViewModel) GetFulfillmentCenter() ReturnsFulfillmentCenterViewModel {
	if o == nil  {
		var ret ReturnsFulfillmentCenterViewModel
		return ret
	}

	return o.FulfillmentCenter
}

// GetFulfillmentCenterOk returns a tuple with the FulfillmentCenter field value
// and a boolean to check if the value has been set.
func (o *ReturnsCreateReturnViewModel) GetFulfillmentCenterOk() (*ReturnsFulfillmentCenterViewModel, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.FulfillmentCenter, true
}

// SetFulfillmentCenter sets field value
func (o *ReturnsCreateReturnViewModel) SetFulfillmentCenter(v ReturnsFulfillmentCenterViewModel) {
	o.FulfillmentCenter = v
}

// GetInventory returns the Inventory field value
// If the value is explicit nil, the zero value for []ReturnsReturnInventoryViewModel will be returned
func (o *ReturnsCreateReturnViewModel) GetInventory() []ReturnsReturnInventoryViewModel {
	if o == nil  {
		var ret []ReturnsReturnInventoryViewModel
		return ret
	}

	return o.Inventory
}

// GetInventoryOk returns a tuple with the Inventory field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ReturnsCreateReturnViewModel) GetInventoryOk() (*[]ReturnsReturnInventoryViewModel, bool) {
	if o == nil || o.Inventory == nil {
		return nil, false
	}
	return &o.Inventory, true
}

// SetInventory sets field value
func (o *ReturnsCreateReturnViewModel) SetInventory(v []ReturnsReturnInventoryViewModel) {
	o.Inventory = v
}

// GetOriginalShipmentId returns the OriginalShipmentId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ReturnsCreateReturnViewModel) GetOriginalShipmentId() int32 {
	if o == nil || o.OriginalShipmentId.Get() == nil {
		var ret int32
		return ret
	}
	return *o.OriginalShipmentId.Get()
}

// GetOriginalShipmentIdOk returns a tuple with the OriginalShipmentId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ReturnsCreateReturnViewModel) GetOriginalShipmentIdOk() (*int32, bool) {
	if o == nil  {
		return nil, false
	}
	return o.OriginalShipmentId.Get(), o.OriginalShipmentId.IsSet()
}

// HasOriginalShipmentId returns a boolean if a field has been set.
func (o *ReturnsCreateReturnViewModel) HasOriginalShipmentId() bool {
	if o != nil && o.OriginalShipmentId.IsSet() {
		return true
	}

	return false
}

// SetOriginalShipmentId gets a reference to the given NullableInt32 and assigns it to the OriginalShipmentId field.
func (o *ReturnsCreateReturnViewModel) SetOriginalShipmentId(v int32) {
	o.OriginalShipmentId.Set(&v)
}
// SetOriginalShipmentIdNil sets the value for OriginalShipmentId to be an explicit nil
func (o *ReturnsCreateReturnViewModel) SetOriginalShipmentIdNil() {
	o.OriginalShipmentId.Set(nil)
}

// UnsetOriginalShipmentId ensures that no value is present for OriginalShipmentId, not even an explicit nil
func (o *ReturnsCreateReturnViewModel) UnsetOriginalShipmentId() {
	o.OriginalShipmentId.Unset()
}

// GetReferenceId returns the ReferenceId field value
// If the value is explicit nil, the zero value for string will be returned
func (o *ReturnsCreateReturnViewModel) GetReferenceId() string {
	if o == nil || o.ReferenceId.Get() == nil {
		var ret string
		return ret
	}

	return *o.ReferenceId.Get()
}

// GetReferenceIdOk returns a tuple with the ReferenceId field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ReturnsCreateReturnViewModel) GetReferenceIdOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.ReferenceId.Get(), o.ReferenceId.IsSet()
}

// SetReferenceId sets field value
func (o *ReturnsCreateReturnViewModel) SetReferenceId(v string) {
	o.ReferenceId.Set(&v)
}

// GetTrackingNumber returns the TrackingNumber field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ReturnsCreateReturnViewModel) GetTrackingNumber() string {
	if o == nil || o.TrackingNumber.Get() == nil {
		var ret string
		return ret
	}
	return *o.TrackingNumber.Get()
}

// GetTrackingNumberOk returns a tuple with the TrackingNumber field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ReturnsCreateReturnViewModel) GetTrackingNumberOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.TrackingNumber.Get(), o.TrackingNumber.IsSet()
}

// HasTrackingNumber returns a boolean if a field has been set.
func (o *ReturnsCreateReturnViewModel) HasTrackingNumber() bool {
	if o != nil && o.TrackingNumber.IsSet() {
		return true
	}

	return false
}

// SetTrackingNumber gets a reference to the given NullableString and assigns it to the TrackingNumber field.
func (o *ReturnsCreateReturnViewModel) SetTrackingNumber(v string) {
	o.TrackingNumber.Set(&v)
}
// SetTrackingNumberNil sets the value for TrackingNumber to be an explicit nil
func (o *ReturnsCreateReturnViewModel) SetTrackingNumberNil() {
	o.TrackingNumber.Set(nil)
}

// UnsetTrackingNumber ensures that no value is present for TrackingNumber, not even an explicit nil
func (o *ReturnsCreateReturnViewModel) UnsetTrackingNumber() {
	o.TrackingNumber.Unset()
}

func (o ReturnsCreateReturnViewModel) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["fulfillment_center"] = o.FulfillmentCenter
	}
	if o.Inventory != nil {
		toSerialize["inventory"] = o.Inventory
	}
	if o.OriginalShipmentId.IsSet() {
		toSerialize["original_shipment_id"] = o.OriginalShipmentId.Get()
	}
	if true {
		toSerialize["reference_id"] = o.ReferenceId.Get()
	}
	if o.TrackingNumber.IsSet() {
		toSerialize["tracking_number"] = o.TrackingNumber.Get()
	}
	return json.Marshal(toSerialize)
}

type NullableReturnsCreateReturnViewModel struct {
	value *ReturnsCreateReturnViewModel
	isSet bool
}

func (v NullableReturnsCreateReturnViewModel) Get() *ReturnsCreateReturnViewModel {
	return v.value
}

func (v *NullableReturnsCreateReturnViewModel) Set(val *ReturnsCreateReturnViewModel) {
	v.value = val
	v.isSet = true
}

func (v NullableReturnsCreateReturnViewModel) IsSet() bool {
	return v.isSet
}

func (v *NullableReturnsCreateReturnViewModel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableReturnsCreateReturnViewModel(val *ReturnsCreateReturnViewModel) *NullableReturnsCreateReturnViewModel {
	return &NullableReturnsCreateReturnViewModel{value: val, isSet: true}
}

func (v NullableReturnsCreateReturnViewModel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableReturnsCreateReturnViewModel) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



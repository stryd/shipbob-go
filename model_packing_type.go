...
// List of PackingType
const (
	PACKINGTYPE_EVERYTHING_IN_ONE_BOX PackingType = "EverythingInOneBox"
	PACKINGTYPE_ONE_SKU_PER_BOX       PackingType = "OneSkuPerBox"
	PACKINGTYPE_MULTIPLE_SKU_PER_BOX  PackingType = "MultipleSkuPerBox"
)

// All allowed values of PackingType enum
var AllowedPackingTypeEnumValues = []PackingType{
	"EverythingInOneBox",
	"OneSkuPerBox",
	"MultipleSkuPerBox",
}
...
func (v *PackingType) UnmarshalJSON(src []byte) error {
	...
	for _, existing := range AllowedPackingTypeEnumValues {
...
// NewPackingTypeFromValue returns a pointer to a valid PackingType
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewPackingTypeFromValue(v string) (*PackingType, error) {
	ev := PackingType(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for PackingType: valid values are %v", v, AllowedPackingTypeEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v PackingType) IsValid() bool {
	for _, existing := range AllowedPackingTypeEnumValues {
		if existing == v {
			return true
		}
